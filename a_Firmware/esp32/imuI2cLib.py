...
class IMUI2C(object):
    ...
    def __init__(self, i2c, addr=0x23, debug=False):
    ...
    def _send_data(self, reg, cmd_data):
        self._i2c.writeto_mem(self._addr, reg, bytes([cmd_data]))
    
    def _send_data_array(self, reg, cmd_array):
        self._i2c.writeto_mem(self._addr, reg, bytes(cmd_array))

    def _read_data(self, reg, num):
        if num > 32:
            num = 32
        data = self._i2c.readfrom_mem(self._addr, reg, num)
        return list(data)
