...
请修复和优化这些问题
1.初始化时未连接usb链接OTG CH340/CH34X 显示为 Switching to MOCK mode; 但是后面插上usb otg连接到驱动板 模式没有切换回来 还是未连接舵机，调试面板的舵机状态也未读取到已连接的舵机；
2.热插拔时同时要扫描所有已连接的舵机，同步显示在调试面板的舵机状态；
3.紧急释放扭矩后再执行动作就没有扭矩了，执行动作时需要开启扭矩；
4.在调试面板其他调试tab：设置id 角度归零 执行角度60度 360度的等舵机的快捷调试按钮 不参与主流程只供单个舵机执行的任务集合